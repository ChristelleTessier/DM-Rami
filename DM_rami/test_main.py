...
def test_egalite(main1, main2, Valeur_attendue):
...
def test_liste_cartes_piocher_resultat(
    main, reserve, Valeur_attendue_main, Valeur_attendue_reserve
):
    main.piocher(reserve)
...
def test_erreur_jeter(main, indice, defausse, type_erreur, message_erreur):
    with pytest.raises(type_erreur, match=re.escape(message_erreur)):
...
def test_liste_cartes_jeter_resultat(
    main, defausse, indice, Valeur_attendue_main, Valeur_attendue_defausse
):
    main.jeter(indice, defausse)
    assert main == Valeur_attendue_main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [[[0, 1, 3]], True, ValueError,
         "(Roi de coeur, Roi de carreau, 8 de coeur) n'est pas une combinaison "
         "valide"],
    ],
)
def test_erreur_poser(
    main_pose, indices_combinaison, premiere_pose, type_erreur, message_erreur
):
    with pytest.raises(type_erreur, match=re.escape(message_erreur)):
...
@pytest.mark.parametrize(
    ...
    [[Main([pytest.deux_coeur, pytest.deux_carreau, pytest.deux_trefle,
            pytest.huit_coeur, pytest.neuf_coeur, pytest.dix_coeur]),
     [[0, 1, 2], [3, 4, 5]],
     ValueError, "Pour la 1ere pose il faut au moins 51 points pour poser"],
...
@pytest.mark.parametrize(
    ...
    [
        [Main(
            [
                pytest.as_carreau, pytest.as_trefle, pytest.as_coeur,
                pytest.deux_carreau
            ]
        ),
         [[0, 1, 3]], ValueError,
...
def test_poser_valide(
    main_pose, indice, Valeur_attendue_main, Valeur_attendue_comb_posee,
...
@pytest.mark.parametrize(
    "main, indice, Valeur_attendue_main, Valeur_attendue_comb_posee,"
    "Valeur_attendue_point",
    [
     [Main(
        [pytest.as_carreau, pytest.as_trefle, pytest.as_coeur, pytest.deux_carreau]
        ), [[0, 1, 2]], Main([pytest.deux_carreau]),
         [
            Combinaison((pytest.as_carreau, pytest.as_trefle, pytest.as_coeur))
        ], 33],],
)
def test_poser_valide2(
    main, indice, Valeur_attendue_main, Valeur_attendue_comb_posee,
    Valeur_attendue_point
):
